...
export const contact = catchAsyncError(async(req,res,next)=>{
    const {name,email,message} = req.body;
     ...
     if(!name || !email || !message) return next(new ErrorHandler("All field are mandatory"))
...
export const courseRequest = catchAsyncError(async(req,res,next)=>{
    ...
    if(!name || !email || !course) return next(new ErrorHandler("All field are mandatory"))
...
export const getDashboardStats = catchAsyncError(async(req,res,next)=>{
    ...
    if(statsData[10].users===0) subscriptionsPercentage = subscriptionsCount*100;
